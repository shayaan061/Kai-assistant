...
export default function ChatLayout() {
  ...
  const fetchHistory = async () => {
    try {
      const res = await fetch("http://127.0.0.1:8000/api/history/1/"); // user_id = 1
  ...
  // ✅ Refresh sidebar ONLY when new conversation is created
  ...
  return (
    <div className="h-screen flex bg-gradient-to-br from-gray-900 via-black to-gray-800 text-white">
      ...
      <aside className="w-72 border-r border-gray-700 p-4 hidden md:flex flex-col gap-4">
        <button
          onClick={() => setActiveId(null)} // Next message will create new chat
